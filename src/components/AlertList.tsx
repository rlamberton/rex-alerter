...
const fetchAlerts = (updateAlerts: Function, setLastUpdatedTime : Function) => {
    const newAlertsPromise = getNewAlerts();
    newAlertsPromise.then((newAlerts) => {
        if (newAlerts && newAlerts.length) {
            // Update the state with the new list
            updateAlerts((oldAlerts: Array<AlertType>) => [...oldAlerts].concat(newAlerts));
 ...
 */
const AlertList : React.FC = function AlertList() {
